...
// - Write a function called areaOfCircle that will take one argument (the radius), calculate the area based on 
//that, and return the result. It should also log a string like "The area for a circle with radius 2 is 
//12.566370614359172."
...
const percentOf = function (num1, num2) {
    const result = Math.floor((num1 / num2) * 100);
...
const areaOfCircle = function (radius) {
    const result = (radius ** 2 * Math.PI).toFixed(2);
...
const multiCalculate = function (num) {
    const halfResult = halfNumber(num);
    const sqResult = squareNumber(halfResult);
    const areaResult = areaOfCircle(sqResult);
    const percentResult = percentOf(areaResult, sqResult);
};
